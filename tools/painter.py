...
if __name__ == "__main__":
    ...
    painted_image = mask_painter(
        input_image, input_mask, mask_color, mask_alpha, contour_color, contour_width
    )
    # save
    painted_image = Image.fromarray(input_image)
    ...
    input_image = np.array(Image.open("images/painter_input_image.jpg").convert("RGB"))
    painted_image_2 = point_painter(
        input_image, input_points, point_color=9, point_radius=20, contour_color=29
    )
    # save
    ...
    image_wo_background = background_remover(
